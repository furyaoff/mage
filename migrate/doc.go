...
We only support migrations between mainnet mage releases.
We only support migrations from the previous mainnet mage version to the current. We don't support migrating between two old versions, use the old software version for this.
...
eg mage-3 migrates `auth.GenesisState` from mage-2 to `auth.GenesisState` from mage-3,
but for mage-4 we don't need to keep around mage-2's `auth.GenesisState` type.
