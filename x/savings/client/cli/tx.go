...
func getCmdDeposit() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s deposit 10000000umage,100000000usdx --from <key>`, version.AppName, types.ModuleName,
		),
...
func getCmdWithdraw() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s withdraw 10000000umage,100000000usdx --from <key>`, version.AppName, types.ModuleName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
