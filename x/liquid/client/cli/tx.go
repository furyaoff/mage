...
func getCmdMintDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s mint magevaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9azvklp 10000000umage --from <key>`, version.AppName, types.ModuleName,
		),
...
func getCmdBurnDerivative() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s burn 10000000bmage-magevaloper16lnfpgn6llvn4fstg5nfrljj6aaxyee9azvklp --from <key>`, version.AppName, types.ModuleName,
		),
