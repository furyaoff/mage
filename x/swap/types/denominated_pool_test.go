...
import (
	...
	types "github.com/furya-official/mage/x/swap/types"
...
func TestDenominatedPool_NewDenominatedPool_Validation(t *testing.T) {
	testCases := []struct {
		...
		{umage(0), usdx(1e6), "reserves must have two denominations: invalid pool"},
		{umage(1e6), usdx(0), "reserves must have two denominations: invalid pool"},
		{usdx(0), umage(1e6), "reserves must have two denominations: invalid pool"},
		{usdx(0), umage(1e6), "reserves must have two denominations: invalid pool"},
	}
...
func TestDenominatedPool_NewDenominatedPoolWithExistingShares_Validation(t *testing.T) {
	testCases := []struct {
		...
		{umage(0), usdx(1e6), i(1), "reserves must have two denominations: invalid pool"},
		{usdx(0), umage(1e6), i(1), "reserves must have two denominations: invalid pool"},
		{umage(1e6), usdx(1e6), i(0), "total shares must be greater than zero: invalid pool"},
		{usdx(1e6), umage(1e6), i(-1), "total shares must be greater than zero: invalid pool"},
	}
...
func TestDenominatedPool_InitialState(t *testing.T) {
	reserves := sdk.NewCoins(umage(1e6), usdx(5e6))
...
func TestDenominatedPool_ShareValue(t *testing.T) {
	reserves := sdk.NewCoins(umage(10e6), usdx(50e6))
...
func TestDenominatedPool_SwapWithExactInput(t *testing.T) {
	...
	assert.Equal(t, sdk.NewCoins(umage(11e6), usdx(45466946)), pool.Reserves())
	...
	assert.Equal(t, sdk.NewCoins(umage(9093390), usdx(55e6)), pool.Reserves())
...
func TestDenominatedPool_SwapWithExactOuput(t *testing.T) {
	...
	assert.Equal(t, sdk.NewCoins(umage(9e6), usdx(55572273)), pool.Reserves())
	...
	assert.Equal(t, sdk.NewCoins(umage(11114456), usdx(45e6)), pool.Reserves())
